...
class Programmer extends Component {
    ...
    componentDidMount() {
        this.props.get_specific_employees({type: "Programmer"});
    }

    render() {
        let list = null;
        if (this.props.reducer.Programmers !== null) {
            list = this.props.reducer.Programmers.map(obj => {
                    return (
        ...
        return (
            <React.Fragment>
                ...
                <Button variant="primary" onClick={()=>{this.props.get_specific_employees({type: "Programmer"})}}>
                    Refresh
                </Button>
            </React.Fragment>
        );
